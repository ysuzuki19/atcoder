...
int main(){
  ...
  cin >> n >> k;
  vector<pair<unsigned long long,int>> friends(n+1);
  friends[0].first = 0;
  friends[0].second = 0;
  for (int i=1; i<n+1; ++i) cin >> friends[i].first >> friends[i].second;

  sort(friends.begin(), friends.end(), [](pair<unsigned long long,int> l, pair<unsigned long long,int> r) { return l.first < r.first; });

  unsigned long long village_id = 0;
  size_t friend_id = 0;
  for (friend_id=1; friend_id<friends.size(); ++friend_id) {
    if (friends[friend_id].first - village_id > k) break;
    k -= (friends[friend_id].first - village_id);
    k += friends[friend_id].second;
    village_id = friends[friend_id].first;
  }

  cout << village_id + k << endl;

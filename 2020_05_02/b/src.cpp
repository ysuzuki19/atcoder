...
int main(){
	...
	unsigned long long right = (16+1) * log(10) / log(1.01);
	while(right - left > 1){
		//unsigned long long center = left + (right - left)/2;
		unsigned long long center = left + (right - left)/2;
		unsigned long long money = 100 * pow(1.01, center);
		cout << setw(11) << money << "   :   ";
		if( money > x ){
			right = center;
			cout << setw(4) << left << " : " << setw(4) << right << endl;
		}else{
			left = center;
			cout << setw(4) << left << " : " << setw(4) << right << endl;
		}

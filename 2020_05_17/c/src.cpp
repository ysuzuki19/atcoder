...
int main(){
	...
	long double a, b, h, m;
	cin >> a >> b >> h >> m;
	long double a_theta = m/60.0 * 2*M_PI;
	long double b_theta = h/12.0 * 2*M_PI + a_theta/12.0;
	long double theta = abs(a_theta - b_theta);
	cout << fixed << setprecision(20) << sqrt( a*a + b*b - 2*a*b*cos(theta) ) << endl;
	return 0;
}

...
int main(){
  ...
  float fractional_part = std::modf(lastmile, &integral_part);
  //cout << integral_part << " " << fractional_part << endl;

  if (-0.0001 < fractional_part && fractional_part < 0.0001 && (int)dist % r == 0) {
    cout << (int)dist/r << endl;
  } else {
    cout << (int)dist/r + 1 << endl;
  }
